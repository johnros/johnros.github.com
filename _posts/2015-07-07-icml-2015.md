...
This can lead to great speedups since after each split, only small subsets of the data are considered for splitting. The deeper the common roots of the bagged forrest, the larger the computational gain.
As a side comment, Taddy also mentioned that for very large data sets (in the scale of eBay), there is not benefit in the variable subsampling the distinguishes between bagging and random forrests. 
...
My take home message: data poisoning attacks are simply an adverserial introduciton of outliers. 
...
I then understood it is non the above. 
...
[Sewoong Oh](http://web.engr.illinois.edu/~swoh/) presented a calculus for computing the privacy vulnerability when performing multiple diferrentially private queries. 
...
This workshop dealt with the issue of learning a large set of labels.

[Jason Weston](https://scholar.google.com/citations?user=lMkTx0EAAAAJ) from Facebook presented the challenge in tagging Facebook posts. 
User hashtags serve as the (weak) labels, and the problem is to provide the $$k$$ most associated tags. 
The ERM problem is thus defined with respect to an ingenious loss he previously presented termed [WSABIE](http://www.australianscience.com.au/research/google/37180.pdf).
...
### Greed is Great
From the greedy optimization algorithms workshop I particularly liked [Aurelien Bellet's](http://perso.telecom-paristech.fr/~abellet/) presentation using the Frank-Wolfe algorithm for similarity learning. 
The fundamental idea is that one the similarity learning problem has been posed as an ERM problem over a space of matrices, it can be efficiently solved using a first order optimizer which advances in the direction of the largest coordinate in the gradient. This is called the Sparse Frank-Wolfe algorithm.
It should also be noted that the matrices are not constrained to correspond to dot products (thus symmetric and PSD) since the similarity metrics being learnt may not be propoer dot products.

### Julia Language
[John Myles White](http://www.johnmyleswhite.com/) presented the [Julia language](https://en.wikipedia.org/wiki/Julia_(programming_language)).
I will not go in detail. If you are unfamilair with the language, you should check it out.
It also got me thinking- when are we going to see machine-learning driven type inference in compilers? 
Asking this question, John said the Javescript compiler already has some probabilistic rules. 
Can ML provide classifications that are reliable enough to be used for type inference within a compiler?
This is obviously reladed to [Leon Bottou's talk](#keynote-leon-bottou) about "weak" and "hard" contracts.


### Resource Efficuent Learning
This workshop fit exactly within my research interests.
I have learned from it a lot. 
A couple of presenters presented loss function which account for the learning cost within the optimization. I admit I have trouble with the units...
It is my feeling that prediction error and estimation cost cannot be compunded in one loss function. 

In a different line of work, [Jake Abernethy](http://web.eecs.umich.edu/~jabernet/) presented a very nice sampling scheme where a reseacher publishes his willingness to pay for each data point, depending on its value. 
Subjects can they decide whether they sell their data or not at the researcher's bidding price.
Perhaps not suprisingly, a researcher's willingness to pay depends on the gradient of the empirical risk at the requested data point.
...
[Ofer Dekel](http://research.microsoft.com/en-us/um/people/oferd/) from Microsoft present a formal framework for simultanous prunning of random forrests. 
I found the theory beatiful with non-intuitive results.
The idea is to prune simultanously a _whole forrest_, and not tree-wise.
...
Several autors have succesfully learned models with $$16$$ and $$8$$ bit precision. 
...
[Cedric Archambeau](http://www0.cs.ucl.ac.uk/staff/c.archambeau/) from Amazon presented the chellenges in [Amazon Machine Learning](https://aws.amazon.com/blogs/aws/amazon-machine-learning-make-data-driven-decisions-at-scale/). 

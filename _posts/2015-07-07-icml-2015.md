...
It should also be noted that the matrices are not constrained to correspond to dot products (thus symmetric and PSD) since the similarity metrics being learnt may not be proper dot products.
...
Asking this question, John said the JavaScript compiler already has some probabilistic rules. 
...
The idea is to prune simultaneously a _whole forrest_, and not tree-wise.
